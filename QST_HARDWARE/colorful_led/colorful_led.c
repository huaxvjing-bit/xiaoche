...
/**************************************************************************
* 函数功能：12颗环形WS2812流水灯控制 (独立非阻塞平行运作)
**************************************************************************/
...
void Ring_Flowing_Led_Step(void)
{
    ...
    for (i = 1; i <= 12; i++)
    {
        /* 环形12灯红白交替: step=0时奇红偶白(1-red,2-white,3-red...); step=1时奇白偶红(每个灯切换至下一个色号) */
        if ((i + Ring_Led_Step) % 2 == 1)
        {
            Set_Ring_Led_Color(i, WS_RED);
        }
        else
        {
            Set_Ring_Led_Color(i, WS_WHITE);
        }
    }
    Refresh_Ring_Led();
...
void Ring_Flowing_Led_Process(void)
{
    ...
    tick_counter++;
    if (tick_counter >= 10) /* 10 * 100ms = 1000ms (每1s切换一次) */
    {
        tick_counter = 0;
        Ring_Flowing_Led_Step();
    }
